...
{   public:
    ...
    int **inc_mat;      //incidential matrix ( equals zero in constructor)
    T * vertex_val;        //each vertex value ( equals zero in constructor)
    bool * is_vertex_checked; //if given vertex is checked ( equals zero in constructor)
    ...
    int ver_to_start;
        ...
        bool EdgeAdd()
        {
            ...
            for(int j=0;j<M;j++)
            {
                std::cout<<"Wprowadz krawedzie grafu skierowanego [A->B] (wierzcholki indeksowane od 0-"<<N-1<<")"<<std::endl;
